...
// Colombia center as default (moves to stops when loaded)
                            ...
                            <div key={ruta.id}
                                ...
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${isEntrada ? "bg-violet-100 text-violet-700" : "bg-blue-100 text-blue-700"}`}>
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                        <div className={`h-full rounded-full transition-all ${isEntrada ? "bg-violet-500" : "bg-blue-500"}`} style={{ width: `${progreso}%` }} />
                                    </div>
                                    <span className="text-[11px] text-slate-400 whitespace-nowrap">
                                        {isEntrada ? (ruta.estado === "FINALIZADA" ? "Recibido" : "Pendiente") : `${ruta.paradas_entregadas}/${ruta.total_paradas}`}
                                    </span>
                                </div>
                            </div>
                        );
                ...
                {selectedRuta ? (
                    <>
                            ...
                            <div className="flex items-center gap-4">
                                <div>
                                    <h2 className="font-bold text-slate-800 text-sm">
                                        {selectedRuta.tipo === "ENTRADA" ? "Pedido de Marca" : "Hoja de Ruta"} #{selectedRuta.id}
                                        {selectedRuta.zona && <span className="ml-2 text-slate-400 font-normal">— {selectedRuta.zona}</span>}
                                    </h2>
                                    <p className="text-xs text-slate-400">
                                        {selectedRuta.fecha}
                                        {selectedRuta.tipo === "ENTRADA" ? ` · Origen: ${selectedRuta.origin_supplier}` : (selectedRuta.transportador && ` · Transportador: ${selectedRuta.transportador}`)}
                        ...
                        <div className="relative" style={{ height: "340px" }}>
                            ...
                            {selectedRuta.paradas.length === 0 && selectedRuta.tipo !== "ENTRADA" && (
                            ...
                            {selectedRuta.tipo === "ENTRADA" && (
                                <div className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-white px-4 py-2 rounded-2xl shadow-xl border border-violet-100 flex items-center gap-3 z-[999]">
                                    <div className="p-2 bg-violet-100 rounded-full"><ArrowDownCircle className="w-5 h-5 text-violet-600" /></div>
                                    <div>
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">Origen: {selectedRuta.origin_supplier}</p>
